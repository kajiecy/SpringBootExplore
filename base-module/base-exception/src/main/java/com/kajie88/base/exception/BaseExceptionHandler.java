...
public class BaseExceptionHandler {
    ...
    public Object resolveException(HttpServletRequest httpServletRequest, Exception e) {
        ...
        respDTO.setRespStatus(ResponseStatus.FAIL);
...
//        if (isAjax(httpServletRequest)) {  fixme 坑判断请求是否为ajax请求
        if (true) {
            ...
            respDTO.setErrorCode(discoverException.getErrorCode());
            respDTO.setErrorMsg(discoverException.getErrorMsg());
            respDTO.setErrorMsgCn(discoverException.getErrorMsgCn());
            respDTO.setErrorTitle(discoverException.getErrorTitle());
            return respDTO;
        } else {
            return "error2";
        }
